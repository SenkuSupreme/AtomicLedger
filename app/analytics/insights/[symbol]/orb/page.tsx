...
export default function ORBMasterclassPage() {
    ...
    // Methodology Sync
    const [viewMode, setViewMode] = useState<'SMC' | 'ICT' | 'ORB' | 'CRT'>('ORB');

    const triggerNewAnalysis = async (force: boolean = false) => {
        setLoading(true);
        const toastId = toast.loading(force ? "Initiating ORB Force Sync..." : "Validating Opening Range Signal...");
        try {
            ...
            if (result && result.smc_advanced) {
                if (!force && data && result.analyzedAt === data.analyzedAt) {
                    toast.success("ORB Setup still valid. Range edges preserved.", { id: toastId });
                } else if (force) {
                    toast.success("ORB Matrix Synchronized. New session range mapped.", { id: toastId });
                } else {
                    toast.info("Range Invalidated. New ORB thesis being generated...", { id: toastId });
                }
                setData(result);
            }
        } catch (e) {
            console.error("Analysis error:", e);
            toast.error("ORB synchronization failed.", { id: toastId });
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-[1800px] mx-auto space-y-12">
                ...
                <header className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-white/5 pb-8 gap-6">
                    <div className="flex items-center gap-5">
                        <div className="flex items-center gap-3">
                            ...
                            <Tooltip text="Validate Range Narrative">
                            ...
                            <Tooltip text="Force ORB Algo Sync">
                ...
                <div className="space-y-10">
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                        <div className="xl:col-span-2 bg-[#0A0A0A] border border-white/5 rounded-[2rem] p-10 h-full flex flex-col">
                        ...
                        <div className="bg-[#0A0A0A] border border-white/5 rounded-[2rem] p-10 h-full flex flex-col">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* ORB SPECIFICS SECTION */}
                    {viewMode === 'ORB' && smc.methodologySpecifics?.orb && (
                        <TechnicalSection title="Opening Range Forensics" icon={Target}>
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                                     <div className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.5rem] flex flex-col items-center">
                                         <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Session</span>
                                         <span className="text-xs font-black text-primary uppercase italic">{smc.methodologySpecifics.orb.session}</span>
                                     </div>
                                     <div className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.5rem] flex flex-col items-center">
                                         <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Duration</span>
                                         <span className="text-xs font-black text-primary uppercase italic">{smc.methodologySpecifics.orb.duration}</span>
                                     </div>
                                     <div className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.5rem] flex flex-col items-center">
                                         <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Range Size</span>
                                         <span className="text-xs font-black text-primary uppercase italic font-mono">{fmt(smc.methodologySpecifics.orb.orh - smc.methodologySpecifics.orb.orl, 4)}</span>
                                     </div>
                                </div>
                                <div className="p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] space-y-4">
                                     <div className="flex justify-between items-center pb-3 border-b border-white/5">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">OR High</span>
                                         <span className="text-sm font-black text-emerald-500 font-mono italic">${fmt(smc.methodologySpecifics.orb.orh)}</span>
                                     </div>
                                     <div className="flex justify-between items-center pb-3 border-b border-white/5">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">OR Midpoint</span>
                                         <span className="text-sm font-black text-muted-foreground font-mono italic">${fmt(smc.methodologySpecifics.orb.midpoint)}</span>
                                     </div>
                                     <div className="flex justify-between items-center">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">OR Low</span>
                                         <span className="text-sm font-black text-rose-500 font-mono italic">${fmt(smc.methodologySpecifics.orb.orl)}</span>
                                     </div>
                                </div>
                            </div>
                        </TechnicalSection>
...
function TechnicalSection({ title, icon: Icon, count, children }: any) {
    return (
        <div className="bg-[#0A0A0A] border border-white/5 rounded-[2.5rem] p-10 relative group overflow-hidden min-h-[400px] flex flex-col h-full">
