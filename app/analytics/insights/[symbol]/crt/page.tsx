...
export default function CRTMasterclassPage() {
    ...
    const triggerNewAnalysis = async (force: boolean = false) => {
        setLoading(true);
        const toastId = toast.loading(force ? "Initiating CRT Force Sync..." : "Validating Contracted Range...");
        try {
            ...
            if (result && result.smc_advanced) {
                if (!force && data && result.analyzedAt === data.analyzedAt) {
                    toast.success("CRT Model still valid. Expansion targets preserved.", { id: toastId });
                } else if (force) {
                    toast.success("CRT Matrix Synchronized. New contracted range identified.", { id: toastId });
                } else {
                    toast.info("Range Reset. New CRT thesis being generated...", { id: toastId });
                }
                setData(result);
            }
        } catch (e) {
            console.error("Analysis error:", e);
            toast.error("CRT synchronization failed.", { id: toastId });
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-[#050505] text-foreground p-4 md:p-12 font-sans pb-32 relative overflow-hidden selection:bg-primary/30">
            ...
            <div className="max-w-[1800px] mx-auto space-y-12">
                ...
                <header className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-white/5 pb-8 gap-6">
                    <div className="flex items-center gap-5">
                        <div className="flex items-center gap-3">
                            ...
                            <Tooltip text="Validate CRT Signal">
                            ...
                            <Tooltip text="Force CRT Algo Sync">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    {/* CRT SPECIFICS SECTION */}
                    {viewMode === 'CRT' && smc.methodologySpecifics?.crt && (
                        <TechnicalSection title="CRT Forensics" icon={Target}>
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 md:grid-cols-2 gap-6">
                                     <div className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.5rem] flex flex-col items-center">
                                         <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Reference Candle</span>
                                         <span className="text-xs font-black text-primary uppercase italic">{smc.methodologySpecifics.crt.referenceCandle}</span>
                                     </div>
                                     <div className="p-4 bg-white/[0.02] border border-white/5 rounded-[1.5rem] flex flex-col items-center">
                                         <span className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-1">Expansion Status</span>
                                         <span className={`text-xs font-black uppercase italic ${smc.methodologySpecifics.crt.expansion ? 'text-emerald-500' : 'text-rose-500'}`}>
                                            {smc.methodologySpecifics.crt.expansion ? 'CONFIRMED' : 'WAITING'}
                                         </span>
                                     </div>
                                </div>
                                <div className="p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] space-y-4">
                                     <div className="flex justify-between items-center pb-3 border-b border-white/5">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">CRT High</span>
                                         <span className="text-sm font-black text-emerald-500 font-mono italic">${fmt(smc.methodologySpecifics.crt.high)}</span>
                                     </div>
                                     <div className="flex justify-between items-center pb-3 border-b border-white/5">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">CRT Midpoint</span>
                                         <span className="text-sm font-black text-muted-foreground font-mono italic">${fmt(smc.methodologySpecifics.crt.midpoint)}</span>
                                     </div>
                                     <div className="flex justify-between items-center">
                                         <span className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">CRT Low</span>
                                         <span className="text-sm font-black text-rose-500 font-mono italic">${fmt(smc.methodologySpecifics.crt.low)}</span>
                                     </div>
                                </div>
                            </div>
                        </TechnicalSection>
